...
@DOC_END */
LightContrib singleLightContribution(Light light, float3 surfacePos, float3 surfaceNormal, float2 randVal)
{
    LightContrib contrib;
    contrib.incidentVector = 0.0F.rrr;
    contrib.halfAngularSize = 0.0F;
    contrib.intensity = 0.0F.rrr;
    ...
    else if (light.type == eLightTypeSpot || light.type == eLightTypePoint)
    {
        ...
        if (light.radius > 0.0F)
        {
            ...
            // A good enough approximation for 2 * (1 - cos(halfAngularSize)), numerically more accurate for small angular
            // sizes
    ...
    if (contrib.halfAngularSize > 0.0F) // <----- Sampling area lights
    { 
        ...
        light_dir = normalize(mul(tbn, dir));
...
otherwise have.
@DOC_END */
LightContrib singleLightContribution(Light light, float3 surfacePos, float3 surfaceNormal)
{
    return singleLightContribution(light, surfacePos, surfaceNormal, 0.0F.rr);
}
