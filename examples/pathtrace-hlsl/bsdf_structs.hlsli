...
// Individual flags:
static const uint BSDF_EVENT_ABSORB = 0; // 0: invalid sample; path should be discarded (radiance 0)
static const uint BSDF_EVENT_DIFFUSE = 1; // 1: e.g. Lambert. Lobe is always centered on the surface normal.
static const uint BSDF_EVENT_GLOSSY =
    (1 << 1); // 2; Center of lobe depends on viewing angle; not perfectly specular reflection.
static const uint BSDF_EVENT_IMPULSE = (1 << 2); // 4; "Perfectly specular" or "mirror-like" reflection or transmission.
static const uint BSDF_EVENT_REFLECTION =
    (1 << 3); // 8; Both view and light directions are on the same side of the geometric normal.
static const uint BSDF_EVENT_TRANSMISSION =
    (1 << 4); // 16; View and light directions are on opposite sides of the geometric normal.

// Combinations:
static const uint BSDF_EVENT_DIFFUSE_REFLECTION = (BSDF_EVENT_DIFFUSE | BSDF_EVENT_REFLECTION); // 9
static const uint BSDF_EVENT_DIFFUSE_TRANSMISSION = (BSDF_EVENT_DIFFUSE | BSDF_EVENT_TRANSMISSION); // 17
static const uint BSDF_EVENT_GLOSSY_REFLECTION = (BSDF_EVENT_GLOSSY | BSDF_EVENT_REFLECTION);       // 10
static const uint BSDF_EVENT_GLOSSY_TRANSMISSION = (BSDF_EVENT_GLOSSY | BSDF_EVENT_TRANSMISSION);   // 18
static const uint BSDF_EVENT_IMPULSE_REFLECTION = (BSDF_EVENT_IMPULSE | BSDF_EVENT_REFLECTION);     // 12
static const uint BSDF_EVENT_IMPULSE_TRANSMISSION = (BSDF_EVENT_IMPULSE | BSDF_EVENT_TRANSMISSION); // 20
