...
> +z is up)
@DOC_END */
float hvd_ggx_eval(const float2 invRoughness,
                          const float3 h) // == vec3(dot(tangent, h), dot(bitangent, h), dot(normal, h))
{
...
@DOC_END */
float ggx_smith_shadow_mask(out float G1, out float G2, const float3 k1, const float3 k2,
...
float2 fresnel_conductor(out float2 phase_shift_sin, out float2 phase_shift_cos, const float n_a, const float n_b,
                         const float k_b, const float cos_a, const float sin_a_sqd)
{
...
float3 thin_film_factor(float coating_thickness, const float coating_ior, const float base_ior,
                               const float incoming_ior, const float kh)
{
    ...
    const float2 R12 =
      fresnel_conductor(phi12_sin, phi12_cos, coating_ior, base_ior, /* base_k = */0.0f, cos1, sin1_sqr);
...
float3 compute_half_vector(const float3 k1, const float3 k2, const float3 normal, const float2 ior,
                                  const float nk2, const bool transmission, const bool thinwalled)
{
    float3 h;
...
float3 refract(const float3 k, // direction (pointing from surface)
                      ...
                      out bool tir)   // total internal reflection
{
...
@DOC_END */
float vcavities_shadow_mask(out float G1, out float G2, const float nh, const float3 k1, const float k1h,
