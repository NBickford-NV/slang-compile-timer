...
Since GLSL doesn't have a distinction between and private functions,
only functions that are part of the "API" will have annotations to
appear in README.md.
...
[the NVIDIA MDL SDK's BSDF
implementations](https://github.com/NVIDIA/MDL-SDK/blob/203d5140b1dee89de17b26e828c4333571878629/src/mdl/jit/libbsdf/libbsdf.cpp).
...
and the sheen lobe ([Conty and Kulla's "Charlie"
sheen](https://blog.selfshadow.com/publications/s2017-shading-course/#course_content) and a V-cavity shadowing-masking
function).

@DOC_END */

#include "constants.hlsli"
#include "func.hlsli"           // cosineSampleHemisphere
#include "ggx.hlsli"            // brdfLambertian, ..
#include "bsdf_structs.hlsli"   // Bsdf*,
#include "pbr_mat_struct.hlsli" // PbrMaterial
...
float3 absorptionCoefficient(PbrMaterial mat) {
  return -log(max(mat.attenuationColor, float3(0.001F, 0.001F, 0.001F))) / max(mat.attenuationDistance, 0.001);
}
...
float[LOBE_COUNT] weights = computeLobeWeights(mat, dot(k1, mat.N), tint).weights;
```
@DOC_END */
LobeWeights computeLobeWeights(PbrMaterial mat, float VdotN, inout float3 tint)
{
  float frCoat = 0.0F;
  ...
  if (mat.iridescence > 0.0f) {
    // When there is iridescence enabled, use the maximum of the estimated iridescence factor. (Estimated with VdotN, no
    // half-vector H here.) With the thinfilm decision this handles the mix between non-iridescence and iridescence
  ...
  */
  ...
  weightLobe[LOBE_SPECULAR_TRANSMISSION] =
      weightBase * mat.transmission; // BTDF dielectric specular transmission (GGX-Smith)
...
@DOC_END */
int findLobe(PbrMaterial mat, float VdotN, float rndVal, out float3 tint) {
  float weightLobe[LOBE_COUNT] = computeLobeWeights(mat, VdotN, tint).weights;
...
void brdf_diffuse_eval(inout BsdfEvaluateData data, PbrMaterial mat, float3 tint) {
...
void brdf_diffuse_eval(inout BsdfEvaluateData data, PbrMaterial mat) { brdf_diffuse_eval(data, mat, mat.baseColor); }

// Samples a diffuse lobe.
void brdf_diffuse_sample(inout BsdfSampleData data, PbrMaterial mat, float3 tint) {
...
void brdf_diffuse_sample(inout BsdfSampleData data, PbrMaterial mat) { brdf_diffuse_sample(data, mat, mat.baseColor); }

void brdf_diffuse_transmission_eval(inout BsdfEvaluateData data, PbrMaterial mat, float3 tint) {
...
void brdf_diffuse_transmission_sample(inout BsdfSampleData data, PbrMaterial mat, float3 tint) {
...
void brdf_ggx_smith_eval(inout BsdfEvaluateData data, PbrMaterial mat, const int lobe, float3 tint) {
  // BRDF or BTDF eval?
  ...
  if (mat.iridescence > 0.0f) {
    ...
    case LOBE_SPECULAR_REFLECTION:
      tint *= lerp(1.F.rrr, factor, mat.iridescence);
      break;
...
// scattering-masking function.
void brdf_ggx_smith_sample(inout BsdfSampleData data, PbrMaterial mat, const int lobe, float3 tint) {
...
See
https://github.com/KhronosGroup/glTF/tree/0251c5c0cce8daec69bd54f29f891e3d0cdb52c8/extensions/2.0/Khronos/KHR_materials_dispersion.
@DOC_END */
...
void btdf_ggx_smith_eval(inout BsdfEvaluateData data, PbrMaterial mat, float3 tint) {
  ...
  const float3 bsdf = (prob * G12 * data.pdf).rrr;
...
void btdf_ggx_smith_sample(inout BsdfSampleData data, PbrMaterial mat, float3 tint) {
  bool isThinWalled = (mat.isThinWalled);
  ...
  data.bsdf_over_pdf = 1.0F.rrr; // Was: (vec3(1.0f) - fr) / prob; // PERF Always white with the original setup.
...
void brdf_sheen_eval(inout BsdfEvaluateData data, PbrMaterial mat) {
  // BRDF or BTDF eval?
  // If the incoming light direction is on the backface.
  // Include edge-on (== 0.0f) as "no light" case.
  const bool backside = (dot(data.k2, mat.Ng) <= 0.0f);
  ...
  const float3 bsdf = (G12 * data.pdf).rrr;
...
void brdf_sheen_sample(inout BsdfSampleData data, PbrMaterial mat) {
  // When the sampling returns eventType = BSDF_EVENT_ABSORB, the path ends inside the ray generation program.
  // Make sure the returned values are valid numbers when manipulating the PRD.
  data.bsdf_over_pdf = 0.0F.rrr;
  data.pdf = 0.0f;
  ...
  data.bsdf_over_pdf = 1.0F.rrr; // PERF Always white with the original setup.
  data.event_type = BSDF_EVENT_GLOSSY_REFLECTION;
  ...
  const float G12 =
      vcavities_shadow_mask(G1, G2, h0.z, k10, k1h, float3(dot(data.k2, mat.T), dot(data.k2, mat.B), nk2), k2h);
  if (G12 <= 0.0f) {
    data.event_type = BSDF_EVENT_ABSORB;
    return;
  }
...
@DOC_END */
void bsdfEvaluate(inout BsdfEvaluateData data, PbrMaterial mat) {
  ...
  if (lobe == LOBE_DIFFUSE_REFLECTION) {
    ...
    mat.roughness = (mat.clearcoatRoughness * mat.clearcoatRoughness).rr;
    mat.N = mat.Nc;
...
@DOC_END */
void bsdfSample(inout BsdfSampleData data, PbrMaterial mat) {
  float3 tint = mat.baseColor;
  float VdotN = dot(data.k1, mat.N);
  int lobe = findLobe(mat, VdotN, data.xi.z, tint);
  data.pdf = 0;
...
@DOC_END */
void bsdfEvaluateSimple(inout BsdfEvaluateData data, PbrMaterial mat) {
  // Specular reflection
  ...
  if (NdotV == 0.0f || NdotL == 0.0f || VdotH == 0.0f || NdotH == 0.0f) {
    data.bsdf_diffuse = data.bsdf_glossy = 0.0F.rrr;
    data.pdf = 0.0f;
    return;
  }
  ...
  float3 f0 = lerp(c_min_reflectance.rrr, mat.baseColor, mat.metallic);
  float3 fGlossy = schlickFresnel(f0, 1.0F.rrr, VdotH); // Metallic + specular
...
the future.
@DOC_END */
void bsdfSampleSimple(inout BsdfSampleData data, PbrMaterial mat) {
  float3 tint = mat.baseColor;
  data.bsdf_over_pdf = 0.0F.rrr;
...
float3 bsdfSimpleAverageReflectance(float3 k1, PbrMaterial mat) {
  ...
  float3 bsdf_glossy_average = schlickFresnel(f0, 1.0F.rrr, NdotV);
